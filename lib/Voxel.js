...
**/
function Voxel(x, y, z, dim, options = {}) {
    let cubeElement;
    let animElement;
    let parentScene;
    let mesh;
    ...
    function setMesh(data = {}) {
        if (data.constructor === Mesh) {
            let old = mesh;
            old && old.removeEventListener('change', onMeshChange);
            mesh = data;
    ...
    function onMeshChange() {
        applyMesh();
        let target = self;
        self.triggerEvent('MeshChange', {target, mesh});
    }
    function onVoxelClick(event) {
        event.preventDefault();
        let target = self;
        self.triggerEvent('VoxelClick', {target});
        return false;
    }
}
