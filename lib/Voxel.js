...
function Voxel(position = [0, 0, 0], size = DEFAULT_SIZE, options = {}) {
    ...
    function createFace(label) {
        ...
        let target = self;
        let handlerLookup = {
            top:    () => self.trigger('TopClick', {target}),
            bottom: () => self.trigger('BottomClick', {target}),
            left:   () => self.trigger('LeftClick', {target}),
            right:  () => self.trigger('RightClick', {target}),
            front:  () => self.trigger('FrontClick', {target}),
            back:   () => self.trigger('BackClick', {target})
        };
        let image = createElement('img', '');
        let shader = createElement('div', 'shader');
        let wrapper = createElement('div', 'voxelcss-face ' + label);
